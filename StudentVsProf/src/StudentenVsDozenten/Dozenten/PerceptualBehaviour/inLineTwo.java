...
//�berpr�t ob in den beiden feldern vor dem Dozent Studenten sind und aktiviert die Schuss funktion
public class inLineTwo extends perceptualBehaviour{
	...
	boolean ShallIshoot() {
		...
		Field F1 = PlayingField.GameMap.getFieldIn((int)P0.getxPosition(),(int)P0.getyPosition());
		f1 = F1.hasStudents();
		}
		Position P1 = myDozent.getPos().addX(1);
		if(PlayingField.GameMap.isPositionInMap(P1)) {
			Field F2 =PlayingField.GameMap.getFieldIn((int)P1.getxPosition(),(int)P1.getyPosition());
			f2 = F2.hasStudents();
		}		
		Position P2 = myDozent.getPos().addX(2);
		if(PlayingField.GameMap.isPositionInMap(P2)) {
			Field F3 =PlayingField.GameMap.getFieldIn((int)P2.getxPosition(),(int)P2.getyPosition());
			f3 = F3.hasStudents();
		}
